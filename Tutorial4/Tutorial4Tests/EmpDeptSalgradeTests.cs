...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        var emps = Database.GetEmps();
         ...
         var result = ( from e in emps group e by e.DeptNo into g 
                                        select new
                                        {
                                            DeptNo = g.Key,
                                            Count = g.Count()
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        var emps = Database.GetEmps();
         ...
         var result = (from e in emps where e.Sal > (from r in emps where r.DeptNo == e.DeptNo
             select r.Sal).Average() select e.EName).ToList();
